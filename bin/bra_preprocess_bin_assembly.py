...
def bwa_mem_mapping(
    ...
    sample_mapping_fq1_Dir:Dict[str,str],
    threads: int
    ...
    """
    # Build BWA index for the new reference genome.
    ...
    for i, fq1_file in enumerate(fq1_files):
        ...
        sample_mapping_fq1_Dir[mapping_fq1] =  fq1_file

        logging.info(f"Mapping info: {fq1_file} to {mapping_fq1}")


        # Run BWA MEM to align sample FASTQs to the reference genome and use samtools view to filter target reads.
        ...
        # Run samtools collate to convert the BAM file to FASTQ.
...
def output_mash_distances(binID:str,mash_dist_dir: Dict[str, float], output_file: str,sample_mapping_fq1_Dir:Dict[str,str],topsamplenum:int):
    """
    Process the mash_dist_dir dictionary and write results to a file.

    :param mash_dist_dir: Dictionary containing mash distance information, with keys as sample names and values as distances.
    :param output_file: Path to the output file.
    """
    # Check if the number of entries in the dictionary is less than or equal to 10
    if len(mash_dist_dir) <= 10:
        # If yes, write all keys to the file
        keys_to_write = mash_dist_dir.keys()
    else:
        # Otherwise, sort the dictionary by values in ascending order and get the top 10 keys
        sorted_keys = sorted(mash_dist_dir, key=mash_dist_dir.get)
        keys_to_write = sorted_keys[:topsamplenum]

    # Write the keys to the output file
    with open(output_file, 'w') as f:
        for key in keys_to_write:
            sampleID = str(sample_mapping_fq1_Dir[key]).split("/")[-1].split("_clean_")[0]
            f.write(f'{binID}\t{sampleID}\t{mash_dist_dir[key]}\n')
...
def filter_fq_by_mash(
    ...
    # Run MASH sketch to pre-compute the FASTQ of the sample with the highest abundance.
    ...
    for i, mapping_fq1_file in enumerate(mapping_fq1_files):
        ...
        # Run MASH sketch to pre-compute the FASTQs of the remaining samples and use MASH dist to calculate the distance between these FASTQs and the highest abundance FASTQ.
    ...
    with open(dist_file) as f:
        for line in f:
            ...
            if not fq2_flag:
                ...
                # fq1 and fq2 are interleaved, indicating that the next line is the distance result for fq2.
                fq2_flag = True
            else:
                ...
                if fq1_dist < max_dist_threshold and fq2_dist < max_dist_threshold:
                    ...
                    print(f'{mapping_fq1_files[i]}\t{fq1_dist}\t{mapping_fq2_files[i]}\t{fq2_dist}')
                    mash_dist_dir[mapping_fq1_files[i]] = fq1_dist + fq2_dist
                i += 1
...
def preprocess_sample_fastqs(
    ...
    """
    # If use_single_sample is True, only use the FASTQ of the highest abundance sample.
    ...
    if not_use_reference:
        run_cmd(f"ln -sf {bin_fasta} {merge_ref_fasta}",description=f'No reference genome is available, so link the bin fa as the reference genome.')
    else:
        cat_fasta_files(ref_fastas + [bin_fasta,], merge_ref_fasta)
    
    sample_mapping_fq1_Dir = {}
    ...
    mash_dist_dir[mapping_fq1_files[0]] = 0

    # Only perform MASH distance filtering if the number of samples is greater than 1.
    if len(mapping_fq1_files) > 1:
        print(
            "\n#############################MASH distance#####################################\n",
            file=sys.stderr,
        )
        # Preprocess: Calculate the MASH distance for each FASTQ against the highest abundance FASTQ, returning samples with dist < max_dist_threshold.
    ...
    # Only keep the Top X samples.
    output_mash_distances(bin_id,mash_dist_dir,Path(f'{outdir}/{bin_id}_Top_{topsamplenum}_sample.txt'),sample_mapping_fq1_Dir,topsamplenum)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Pick mash distance minium top X sample for assembly. Default: [%(default)s]",
    )
    parser.add_argument(
        "-t",
        "--threads",
    ...
    topSampleNum = args.topSampleNum

    
    if "N/A" in args.reference_fasta :
        ref_fastas = []
        not_use_reference = True
        print(args.reference_fasta)

    else:
        ref_fastas = [Path(i).resolve() for i in args.reference_fasta]
        not_use_reference = args.not_use_reference
    ...
    logging.info(f"reference fasta: {ref_fastas}")
    ...
    logging.info(f"Minimum mash distance Top sample threshold: {topSampleNum}")

    # Remove the max abundance sample from the samples FASTQ before re-inputting the function.
    samples_fastq1.remove(max_abundance_fq1)
    samples_fastq2.remove(max_abundance_fq2)
    print(samples_fastq1)
